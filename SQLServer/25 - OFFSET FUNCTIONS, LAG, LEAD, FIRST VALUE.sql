...
SELECT
	*,
	...
	FORMAT(1.0*(SUM(Quantidade_Vendida) OVER(PARTITION BY Marca))/SUM(Quantidade_Vendida) OVER(), '0.00%') AS '% PARTICIPACAO'
FROM vwProdutos
ORDER BY Marca
...
SELECT
	*,
	ISNULL(SUM(NOVOS_CLIENTES) OVER(ORDER BY ID ROWS BETWEEN 2 PRECEDING AND CURRENT ROW), 0) AS 'MOVING SUM'
FROM
	vwNovosClientes
-- 7 B)
SELECT
	*,
	ISNULL(AVG(NOVOS_CLIENTES) OVER(ORDER BY ID ROWS BETWEEN 2 PRECEDING AND CURRENT ROW), 0) AS 'MOVING SUM'
FROM
	vwNovosClientes
-- 7 C)
SELECT
...
SELECT
	*,
	FORMAT(ISNULL(CAST(NOVOS_CLIENTES AS FLOAT)/NULLIF(LAG(CAST(NOVOS_CLIENTES AS FLOAT), 1, 0) OVER(ORDER BY ID), 0)-1, 0), '0.00%') AS 'MoM',
	FORMAT(ISNULL(CAST(NOVOS_CLIENTES AS FLOAT)/NULLIF(LAG(CAST(NOVOS_CLIENTES AS FLOAT), 1, 0) OVER(PARTITION BY NOMEMES ORDER BY ANO), 0)-1, 0), '0.00%') AS 'YoY'
FROM
	vwNovosClientes
ORDER BY ID
